...
class imagenes extends CI_Controller {
        ...
        function verImagenesPagoCliente($idViaje)
	{
        ...
        function pagoProveedor($idPago, $monto)
	{
              	$image_crud = new image_CRUD();	
                
                $this->session->set_userdata('titulo', "Pago de proveedores - Imagenes de la factura ".$idPago." - Monto $".$monto. " (Tamaño máximo imagen: 1024 x 768)"); 
                
                $image_crud->set_language('spanish');
		
		$image_crud->set_table('imagenes_pago_proveedor')
		->set_relation_field('id_pago')
		->set_ordering_field('orden')
		->set_image_path('assets/uploads/pagos_clientes');
